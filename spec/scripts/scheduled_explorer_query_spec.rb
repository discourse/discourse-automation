...
describe "ScheduledExplorerQuery" do
  fab!(:automation) do
    Fabricate(
      :automation,
      script: DiscourseAutomation::Scriptable::SCHEDULED_EXPLORER_QUERY,
      trigger: "recurring",
    )
  end
  ...
  let!(:recipients) { %w[johndoe john@doe.com] }
  ...
  context "when using recurring trigger" do
    ...
    it "should ensure that script creator with send to email permissions, can send pms to emails" do
      ...
      expect(automation.trigger!).to eq(%w[johndoe john@doe.com])
    end
